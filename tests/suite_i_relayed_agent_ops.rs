...
async fn test_relayed_agent_operations() {
    ...
    let health = client.get(format!("{}/health", RELAYER_URL))
        .send().await.expect("Relayer health check failed");
    ...
    if !stderr.is_empty() { println!("Register stderr: {}", stderr); }

    assert!(output.status.success(), "Registration script failed:\n{}", stderr);
    assert!(stdout.contains("Relayed Transaction Sent"), "Should use relay path");
    ...
    // Verify on-chain: query get_agent_id via HTTP
    let agent_sc_addr = Address::from_slice(agent_wallet.to_address().as_bytes());
    let vm_query = json!({
        "scAddress": registry_addr_bech32,
        "funcName": "get_agent_id",
        "args": [hex::encode(agent_sc_addr.as_bytes())]
    });
    ...
    println!("get_agent_id response: {}", serde_json::to_string_pretty(&vm_body["data"]["data"]).unwrap_or_default());

    let return_code = vm_body["data"]["data"]["returnCode"].as_str().unwrap_or("unknown");
    assert_eq!(return_code, "ok", "get_agent_id query failed: {}", vm_body);

    let return_data = vm_body["data"]["data"]["returnData"].as_array()
        .expect("returnData not found");

    // returnData should have a non-empty entry (the agent's NFT nonce as base64)
    let has_agent = return_data.iter().any(|v| {
        v.as_str().map_or(false, |s| !s.is_empty())
    });
    assert!(has_agent, "Agent should have a non-zero ID after registration. returnData: {:?}", return_data);
    ...
    println!("Relayer Address for agent shard: {}", relayer_address_bech32);
    ...
    let agent_nonce = agent_nonce_res["data"]["account"]["nonce"].as_u64().unwrap_or(0);
    ...
    let sign_output = std::process::Command::new("node")
        ...
        .arg(format!(r#"
            ...
            const {{ Address, TransactionComputer, SmartContractTransactionsFactory, TransactionsFactoryConfig, Abi }} = require('@multiversx/sdk-core');
            const fs = require('fs');
            ...
            async function main() {{
                ...
                // set_metadata(nonce: u64, entries: List<MetadataEntry>)
                // nonce = agent NFT nonce (1), entries = list of MetadataEntry structs
                const agentNftNonce = BigInt(1);
                const entries = [
                    {{ key: Buffer.from('price:default'), value: Buffer.from('0de0b6b3a7640000', 'hex') }},
                    {{ key: Buffer.from('token:default'), value: Buffer.from('EGLD') }}
                ];

                const tx = await factory.createTransactionForExecute(sender, {{
                    contract: new Address('{}'),
                    function: 'set_metadata',
                    arguments: [agentNftNonce, entries],
                    gasLimit: 6000000n,
                }});
    ...
    if !sign_output.status.success() {
        panic!("Signing set_metadata failed:\nstdout: {}\nstderr: {}", sign_stdout, sign_stderr);
    }

    let signed_tx_str = sign_stdout.lines().last()
    ...
    let relay_res = client.post(format!("{}/relay", RELAYER_URL))
    ...
    assert!(relay_status.is_success(), "set_metadata relay failed: {}", relay_body);
    assert!(relay_body["txHash"].is_string(), "Response should contain txHash");
    println!("✅ set_metadata via Relayer: broadcast SUCCESS (txHash: {})", relay_body["txHash"]);
    ...
    let return_data2 = vm_body2["data"]["data"]["returnData"].as_array()
        .expect("returnData not found");

    // Decode agent nonce from base64
    let agent_nonce_b64 = return_data2[0].as_str().unwrap_or("");
    if !agent_nonce_b64.is_empty() {
        let nonce_bytes = base64::engine::general_purpose::STANDARD.decode(agent_nonce_b64).unwrap();
        ...
        let agent_return_code = vm_agent_body["data"]["data"]["returnCode"].as_str().unwrap_or("unknown");
